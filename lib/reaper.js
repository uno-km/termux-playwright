...
class ProcessReaper {
    ...
    static _setupSignalHandlers() {
        ...
        // Fail-safe cleanup on unhandled error crashes
        process.on('uncaughtException', (err) => {
            try {
                ProcessReaper.killAllTracked();
            } catch (e) {}
            console.error('[Termux-Playwright] Uncaught Exception crash:', err);
            process.exit(1);
        });
        ...
        process.on('unhandledRejection', (reason, promise) => {
            ...
            console.error('[Termux-Playwright] Unhandled Rejection crash:', reason);
        });
    }
}
