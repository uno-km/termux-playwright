...
const { TunnelProxy, queryDnsA } = require('./tunnel');
...
function buildChromiumArgs(options = {}) {
    ...
    if (options.tunnelPort) {
        defaultFlags.push(`--proxy-server=http://127.0.0.1:${options.tunnelPort}`);
    }
...
async function launch(playwrightInstance = null, options = {}) {
    ...
    const bypassTunnel = options.bypassTunnel === true || options.tunnelBypass === true || options.isTunnelBypass === true;
    const dnsServer = options.dnsServer || options.dns || '8.8.8.8';
    let tunnelProxy = null;
    let tunnelPort = null;
    if (bypassTunnel) {
        tunnelProxy = new TunnelProxy(dnsServer);
        tunnelPort = await tunnelProxy.start();
    }

    const launchArgs = buildChromiumArgs(Object.assign({}, options, { sessionToken, tunnelPort }));
...
/**
 * Fluent configuration builder for Termux Playwright browser instances.
 */
class BrowserBuilder {
    ...
    headless(enabled = true) {
        this._options.headless = enabled;
        return this;
    }

    lowMemory(enabled = true) {
        this._options.lowMemoryMode = enabled;
        return this;
    }

    jitless(enabled = true) {
        this._options.jitless = enabled;
        return this;
    }

    ignoreCertificateErrors(enabled = true) {
        this._options.ignoreCertificateErrors = enabled;
        return this;
    }

    standaloneMode(enabled = true) {
        this._options.standaloneMode = enabled;
        return this;
    }

    wakeLock(enabled = true) {
        this._options.wakeLock = enabled;
        return this;
    }

    stealth(enabled = true) {
        this._options.stealth = enabled;
        return this;
    }

    singleProcess(enabled = true) {
        this._options.singleProcess = enabled;
        return this;
    }

    bypassTunnel(enabled = true, dnsServer = '8.8.8.8') {
        this._options.bypassTunnel = enabled;
        if (dnsServer) {
            this._options.dnsServer = dnsServer;
        }
        return this;
    }

    isTunnelBypass(enabled = true, dnsServer = '8.8.8.8') {
        return this.bypassTunnel(enabled, dnsServer);
    }
    ...
    withArgs(...args) {
        this._options.args = (this._options.args || []).concat(args);
        return this;
    }
    ...
    async launch(playwrightInstance = null) {
        return await launch(playwrightInstance, this._options);
    }
}
