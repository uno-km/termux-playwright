...
module.exports = {
    ...
    BrowserBuilder: browser.BrowserBuilder,

    // Direct Socket DNS & Tunnel Proxy
    TunnelProxy: require('./tunnel').TunnelProxy,
    queryDnsA: require('./tunnel').queryDnsA,

    // Diagnostics & Doctor Aliases
    doctor: () => {
        const bin = platform.findChromiumBinary();
        const node = platform.findNodeBinary();
        return !!(bin && node);
    },
    runDoctorHealthCheck: () => {
        const bin = platform.findChromiumBinary();
        const node = platform.findNodeBinary();
        return !!(bin && node);
    }
